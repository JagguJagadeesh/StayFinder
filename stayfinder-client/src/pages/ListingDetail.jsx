...
function ListingDetail() {
  ...
  useEffect(() => {
    async function fetchDetails() {
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/api/listings/${id}`);
        setDetails(res.data);
      } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gradient-to-bl from-pink-50 to-pink-300">
      {/* Left side */}
      <div className="w-1/2 md:w-full p-6 ml-6">
        <h1 className="text-3xl font-bold mb-2">{details.title}</h1>
        <p className="text-gray-600 mb-1"><span className="text-lg text-black font-medium">Location:</span> {details.location}</p>
        <p className="text-gray-700 mb-4"><span className="text-lg text-black font-medium">Description:</span> {details.description}</p>
        <p className="text-3xl font-semibold text-red-950 mb-6"><span className="text-green-500">{details.price}</span>₹ <span className="text-sm">/ night</span></p>
        <ImageCarousel images={details.images} />
      </div>

      {/* Right side */}
      <div className="w-1/2 md:w-full p-10">
        <Link to={'/'}><h1 className="text-5xl font-light text-center text-pink-600 mb-10">StayFinder</h1></Link>
        <div className="w-full shadow-2xl p-8 bg-pink-200 border-r border-b border-pink-600 rounded-lg max-w-md mx-auto">
          <h2 className="text-2xl text-center font-mono mb-4">Book This Stay</h2>
